...
class ResNet_IBN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x) # 160 * 120
        feature.append(self.bn1(x))
        x = self.bn1(x)
        x = self.relu(x)

        x = self.maxpool(x)  # 80 * 60

        x = self.layer1([x, feature])
        low_level_feat = x[0]

        x = self.layer2([x[0], x[1]])
        x = self.layer3([x[0], x[1]])
        x = self.layer4([x[0], x[1]])
...
def resnet34_ibn_b(pretrained=True, **kwargs):
...
def resnet101_ibn_b(pretrained=True, **kwargs):
    ...
    '''
    if pretrained:
        pretrain_dict = torch.hub.load_state_dict_from_url(model_urls['resnet101_ibn_b'])
        # model_dict = {}
        # state_list = ["layer1.0.IN.weight", "layer1.0.IN.bias", "layer1.1.IN.weight", "layer1.1.IN.bias", "layer2.0.IN.weight", "layer2.0.IN.bias", "layer2.1.IN.weight", "layer2.1.IN.bias", "layer2.2.IN.weight", "layer2.2.IN.bias",
        #               "fc.weight", "fc.bias", "layer1.2.IN.weight", "layer1.2.IN.bias", "layer2.3.IN.weight", "layer2.3.IN.bias"]
