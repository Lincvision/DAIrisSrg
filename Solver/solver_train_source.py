...
class Solver(object):
    ...
    def tsne(self):
        ...
        with (torch.no_grad()):
            ...
            for i, (images, GT, filename, width, length) in enumerate(self.test_loader):
                ...
                selected_clsid = [0, 1]
                sequence_of_colors = ["tab:red", "tab:cyan"]
                x_last = torch.sigmoid(x_last)
                x_last[x_last >= 0.5] = 1
                x_last[x_last< 0.5] = 0
                GT = F.interpolate(GT.float(), size=(x_to_last.size(2), x_to_last.size(3)), mode='bilinear',align_corners=False)  # torch.Size([32, 1, 75, 100])
                GT = GT.to(torch.int64)
                for index in range(images.size(0)):

                    tsne_file_path =os.path.join(self.result_path, filename[index] + '.png')
                    gt_cuda = GT[index]
                    gt = gt_cuda.view(-1)
                    features = x_to_last[index].view(x_to_last.size(1), -1)
                    features = features.t() # 7500 * 304
                    feat_vecs_temp = features.cpu().numpy()
                    feat_vec_labels_temp = gt.cpu().numpy()  # (7500,)
                    vecs2tsne = feat_vecs_temp  # (7500, 304)

                    assert len(feat_vecs_temp.shape) == 2
                    X_embedded = self.TSNE.fit_transform(vecs2tsne)  # (5600, 2)
                    print('\ntsne done')
                    ...
                    for cls_i in [0, 1]:
                        ...
                        ax.scatter(temp_coords[:, 0], temp_coords[:, 1],
                                   ...
                                   label=sequence_of_colors[cls_i] + '_' + self.trainId2name[cls_i], s=10,
                                   marker='o')
                        # ax.set_axis_off()
                    # plt.show()
    ...
    def test_2(self):
        ...
        for i, (images, filename, width, length) in enumerate(self.test_loader):
            ...
            # 保存分割结果为png文件
            prediction, x_to_last , _ = self.net(images)

            x_visualize = x_to_last  # torch.Size([32, 256, 10, 13])
            x_visualize = F.interpolate(x_visualize, size=(length, width), mode='bilinear')
            x_visualize = x_visualize.detach().cpu().numpy()  # 用Numpy处理返回的[1,256,513,513]特征图
            x_visualize = np.mean(x_visualize, axis=1).reshape(length, width)  # shape为[513,513]，二维
            x_visualize = (((x_visualize - np.min(x_visualize)) / (
                        np.max(x_visualize) - np.min(x_visualize))) * 255).astype(np.uint8)
            x_visualize = cv2.applyColorMap(x_visualize, cv2.COLORMAP_JET)
            fn = os.path.join(self.result_path, str(*filename) + '.png')
            cv2.imwrite(fn, x_visualize)
